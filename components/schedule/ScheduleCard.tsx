...
export function ScheduleCard({ title, time, isActive = false }: ScheduleCardProps) {
  return (
    <div
    ...
    >
      ...
      <div className="grow flex flex-col gap-1.5">
        <div className={`${isActive ? "text-white opacity-90" : "text-black opacity-70"} text-base font-medium`}>
          {title}
        </div>
        <div className={`${isActive ? "text-white opacity-60" : "text-black opacity-50"} text-sm font-medium`}>
          {time}
        </div>
      </div>
      {isActive && (
        <button className="px-4 py-1.5 bg-white rounded-full shadow-sm hover:bg-gray-50 transition-colors">
          <span className="text-[#101010] text-sm font-medium">
            Join now
          </span>
        </button>
      )}
